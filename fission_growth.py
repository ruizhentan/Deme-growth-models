...
def therapy_AT(Time, cap, grid_len, r_s, r_R, d0, fs, fr, 
    ...
    for t in range(1,T): 
        ...
        if (drug == 1):
            if((np.sum(Ns_temp1[:,:] + Nr_temp1[:,:]))< int(0.75*N_initial)):
                drug = 0
        else:
...
## 2 nests #expect not to make a difference. 
...
def therapy_AT_4loc(Time, cap, grid_len, r_s, r_R, d0, fs, fr, 
               s_spread, r_spread, r_shift, d_r, d_D, filestring):
    del_t = 0.01
    T = int(Time/(del_t))
    Ns = np.zeros((grid_len, grid_len,Time)).astype(int)
...
def therapy_CT_4loc(Time, cap, grid_len, r_s, r_R, d0, fs, fr, 
               s_spread, r_spread, r_shift, d_r, d_D, filestring):
    del_t = 0.01
    T = int(Time/(del_t))
    Ns = np.zeros((grid_len, grid_len,Time)).astype(int)
